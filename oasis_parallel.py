...
def prepare_oasis3_dataset(data_path, save_artefacts=False, category='train',
    ...
    for i in range(len(data_combined)):
        if fmt == 'jpg':
            img = Image.fromarray((data_combined[i]).astype('uint8'))
            img.save(save_path.format(i))
        else:
...
def main():
    ...
    #     category='train', max_scans=24, skip=0, fmt='jpg')
    # prepare_oasis3_dataset('/home/ben/projects/honours/datasets/oasis3/exp2', 
    #     category='valid', max_scans=6, skip=24, fmt='jpg')
    # prepare_oasis3_dataset('/home/ben/projects/honours/datasets/oasis3/exp2', 
    #     category='test', max_scans=9, skip=30, fmt='jpg')

    view_slices(path, slices=[(120, 180)], max_scans=10, skip=2, 
